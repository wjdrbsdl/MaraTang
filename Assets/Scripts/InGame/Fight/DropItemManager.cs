...
public class DropItemManager : Mg<DropItemManager>
{
    ...
    private List<int> equiptPidList;
    ...
    {
        equiptPidList = new();
        foreach(KeyValuePair<int, EquiptItemData> item in MgMasterData.GetInstance().GetEquiptDataDic())
        {
            equiptPidList.Add(item.Key);
        }
    }
    #endregion
    ...
    private void FixedDrop(List<TOrderItem> _aquireList, List<TOrderItem> _fixedDropList)
    {
        for (int i = 0; i < _fixedDropList.Count; i++)
        {
            ...
            if (dropItem.Tokentype.Equals(TokenType.Equipt))
            {
               ...
               // Debug.Log(dropItem.SubIdx + "번 장비" + dropItem.Value + "티어로 드랍");
